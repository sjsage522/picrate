...
public interface RatingScoreRepository extends JpaRepository<RatingScore, Long> {
    @Query(value = "SELECT new com.junseok.picrate.rating.dto.RatingStatisticsResponse(rs.rating.id, rs.rating.label, AVG(rs.rate), SUM(rs.rate)) FROM RatingScore rs WHERE rs.card.id = :card_id GROUP BY rs.rating.id")
